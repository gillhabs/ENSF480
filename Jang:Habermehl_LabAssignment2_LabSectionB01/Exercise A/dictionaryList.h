...
public:
  ...
  ~DictionaryList();
    ...
    friend ostream& operator <<(ostream& os, DictionaryList& d);
    
    // Overloading function declarations
    
    const Mystring& operator[](int index) const; // [] overloader
    // REQUIRES: index is an index within the DictionaryList
    // PROMISES: Returns a constant reference to the Mystring at the index
    
    Mystring& operator[](int index); // [] overloader
    // REQUIRES: index is an index within the DictionaryList
    // PROMISES: Returns a reference to the Mystring at the index
    // Allows for modification
    
    Mystring& cursor_datum(); // cursor_datum() redefinition for const version
    // REQUIRES: cursor_ok()
