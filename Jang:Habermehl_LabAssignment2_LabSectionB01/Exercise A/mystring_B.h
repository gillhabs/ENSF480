...
public:
  ...
  //           of rhs.
    ...
    // Declare ostream << overloader a friend
    friend ostream& operator <<(ostream& os, const Mystring& s);
    
    // Overloading function declarations
    
    bool operator >=(const Mystring& s) const; // >= overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true current is greater than or equal to s

    bool operator <=(const Mystring& s) const; // <= overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true current is less than or equal to s
    
    bool operator !=(const Mystring& s) const; // != overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true if current is not equal to s
    
    bool operator ==(const Mystring& s) const; // == overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true if current is equal to s
    
    bool operator >(const Mystring& s) const; // > overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true current is greater than s
    bool operator <(const Mystring& s) const; // < overloader
    // REQUIRES: s is a reference to a Mystring
    // PROMISES: Returns true current is less than s

    char operator[](int index) const; // [] overloader
    // REQUIRES: index is an index within the Mystring
    // PROMISES: Returns the char at the index
    
    char& operator[](int index); // [] overloader
    // REQUIRES: index is an index within the Mystring
    // PROMISES: Returns a reference to the char at the index
